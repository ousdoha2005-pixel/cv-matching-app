...
job_text_input = st.text_area(t("Or paste job description", "Ou coller l'offre"))
...
if cv_text.strip() == "" or job_text.strip() == "":
    ...
    c1.markdown(f"<div class='card'>🎯 {prediction}</div>", unsafe_allow_html=True)
    ...
    cols = st.columns(5)
    for i, skill in enumerate(common):
        cols[i % 5].markdown(f"<div class='skill match'>{skill}</div>", unsafe_allow_html=True)

    st.subheader("Missing Skills")
    cols = st.columns(5)
    for i, skill in enumerate(missing):
        cols[i % 5].markdown(f"<div class='skill missing'>{skill}</div>", unsafe_allow_html=True)
    ...
    def generate_pdf():
        ...
        content = [
            Paragraph(f"Category: {prediction}", styles["Normal"]),
            Paragraph(f"Score: {score10}/10", styles["Normal"]),
